...
class TestBuild(BaseTestCase):
    def setUp(self):
        ...
        self.get_request_response = [{'id': 222325240,
                                      'status': 'success',
                                      'stage': 'test',
                                      'name': 'unit test',
                                      'ref': 'master',
    ...
    @patch('gitlab.build.views.Build.get_request')
    def test_view_get_project_build(self, mocked_build_get_request):
        mocked_build_get_request.return_value = self.get_request_response
    ...
    @patch('gitlab.build.views.Build.get_request')
    def test_view_get_project_build_http_error(self, mocked_build_get_request):
        mocked_build_get_request.side_effect = HTTPError
        chat_id = "8212"
    ...
    @patch('gitlab.build.views.Build.get_request')
    def test_get_project_build(self, mocked_build_get_request):
        mocked_build_get_request.return_value = self.get_request_response
